...
static void ssh_none_free(struct git_cred *cred)
{
        git_cred_ssh_none *c = (git_cred_ssh_none *)cred;
...
int git_cred_ssh_none_new(git_cred **cred, const char *username)
{
        git_cred_ssh_none *c;

        assert(cred);

        c = git__calloc(1, sizeof(git_cred_ssh_none));
        GITERR_CHECK_ALLOC(c);

        c->parent.credtype = GIT_CREDTYPE_SSH_NONE;
        c->parent.free = ssh_none_free;
